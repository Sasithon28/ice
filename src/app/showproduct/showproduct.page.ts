...
import { IonContent, IonHeader, IonTitle, IonToolbar, IonButton, IonLabel, IonCard, IonCardSubtitle, IonCardTitle, IonCardHeader, AlertController } from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [ IonCardHeader, IonCard, IonContent, IonHeader, IonTitle, IonToolbar, CommonModule, FormsModule, IonCardSubtitle, IonCardSubtitle, IonCardTitle, IonButton, IonLabel],
})
export class ShowproductPage implements OnInit {
  products: any = [];
  baseUrl = 'http://localhost/class2-4/crudapi/';
  ...
  updateProduct(id: number, updatedData: { name: string; price: number }, newImage?: File) {
    ...
    this.dataapi.editproduct(formData).subscribe({
        ...
        if (response.status === 'success') {
          const alert = await this.alertController.create({
            ...
            buttons: [{
              text: 'ตกลง',
              handler: () => {
                this.showproduct(); // Reload the products list
              }
            }]
          });
          await alert.present();
